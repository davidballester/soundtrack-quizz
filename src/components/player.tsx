...
export function Player({
  ...
  const { player, started } = usePlayer({ videoId });
  ...
  return (
    <div className={`player ${started ? "started" : ""}`}>
      ...
      <iframe
        ...
        width={width}
        height={height}
        src={`http://www.youtube.com/embed/${videoId}?enablejsapi=1&origin=${location.origin}&autoplay=1&controls=0&fs=0&playsinline=1&disablekb=1&color=white`}
      ></iframe>
    </div>
  );
}
